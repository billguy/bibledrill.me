class VersesController < ChaptersController
  ...
  def show
    ...
    add_breadcrumb "Chapter #{@chapter.number}", book_chapter_path(book_id: @book.permalink, chapter_id: @chapter.number), title: "Chapter #{@chapter.number} verses", remote: true
    if @verses.length > 1
        @page_title = @chapter.title
    else
